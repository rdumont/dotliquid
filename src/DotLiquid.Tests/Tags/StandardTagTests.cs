...
namespace DotLiquid.Tests.Tags
{
	...
	public class StandardTagTests
	{
		...
		{
			Tag tag = new Tag();
			tag.InitializeAsync("tag", null, null).Wait();
			Assert.AreEqual("tag", tag.Name);
			Assert.AreEqual(string.Empty, tag.RenderAsync(new Context()).Result);
		}
...
{%endfor%}";
		...
		[Test]
		public async Task TestCaptureDetectsBadSyntax()
		{
			Assert.Throws<SyntaxException>(async () =>
				await Helper.AssertTemplateResultAsync("content foo content foo ", "{{ var2 }}{% capture %}{{ var }} foo {% endcapture %}{{ var2 }}{{ var2 }}", Hash.FromAnonymousObject(new { var = "content" })));
		}
		...
		public async Task TestAssignFromCase()
		{
			...
			Template template = Template.ParseAsync(code).Result;
            Assert.AreEqual("menswear", template.RenderAsync(Hash.FromAnonymousObject(new { collection = new { handle = "menswear-jackets" } })).Result);
            Assert.AreEqual("menswear", template.RenderAsync(Hash.FromAnonymousObject(new { collection = new { handle = "menswear-t-shirts" } })).Result);
            Assert.AreEqual("womenswear", template.RenderAsync(Hash.FromAnonymousObject(new { collection = new { handle = "x" } })).Result);
            Assert.AreEqual("womenswear", template.RenderAsync(Hash.FromAnonymousObject(new { collection = new { handle = "y" } })).Result);
            Assert.AreEqual("womenswear", template.RenderAsync(Hash.FromAnonymousObject(new { collection = new { handle = "z" } })).Result);
		}
		...
		{
            Assert.AreEqual("variable", Template.ParseAsync("{% assign a = 'variable' %}{{a}}").Result.RenderAsync().Result);
		}

		[Test]
		public async Task TestAssignAnEmptyString()
		{
            Assert.AreEqual("", Template.ParseAsync("{% assign a = '' %}{{a}}").Result.RenderAsync().Result);
		}

		[Test]
		public async Task TestAssignIsGlobal()
		{
            Assert.AreEqual("variable", Template.ParseAsync("{%for i in (1..2) %}{% assign a = 'variable'%}{% endfor %}{{a}}").Result.RenderAsync().Result);
		}

		[Test]
		public async Task TestCaseDetectsBadSyntax()
		{
			Assert.Throws<SyntaxException>(async () => await Helper.AssertTemplateResultAsync("", "{% case false %}{% when %}true{% endcase %}", new Hash()));
			Assert.Throws<SyntaxException>(async () => await Helper.AssertTemplateResultAsync("", "{% case false %}{% huh %}true{% endcase %}", new Hash()));
		}
