	...
	public class IfElseTests
	{
        ...
        public void TestComparisonOfStringsContainingAndOrOr()
		{
			Assert.DoesNotThrow(async () =>
			{
				...
				await Helper.AssertTemplateResultAsync(" YES ", "{% if " + awfulMarkup + " %} YES {% endif %}", assigns);
			});
		}
		...
		{
			Assert.Throws<SyntaxException>(async () => await Helper.AssertTemplateResultAsync("", "{% if jerry == 1 %}"));
		}

		[Test]
        public void TestSyntaxErrorNoExpression()
		{
			Assert.Throws<SyntaxException>(async () => await Helper.AssertTemplateResultAsync("", "{% if %}"));
		}
